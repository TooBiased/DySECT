//#include "include/spacegrow.hpp"
#include "selection.hpp"

// #include "include/strategies/dstrat_bfs.hpp"
// #include "include/strategies/dstrat_rwalk.hpp"
// #include "include/strategies/dstrat_rwalk_cyclic.hpp"
...
template <class Config> struct Test
{
    ...
    int operator()(size_t it, size_t n, size_t pre, size_t cap, size_t pattern,
                   size_t steps, double alpha)
    {
        otm::out() << otm::width(4) << "# it" << otm::width(8) << "alpha";
        table_type::print_init_header(otm::out());
        otm::out() << otm::width(9) << "cap" << otm::width(4) << "pat"
                   << otm::width(9) << "pre" << otm::width(9) << "n"
                   << otm::width(8) << "t_pre" << otm::width(8) << "t_mix"
                   << otm::width(9) << "in_err"
#ifdef MALLOC_COUNT
                   << otm::width(7) << "memory"
#endif
        ...
        for (size_t i = 0; i < pre; ++i) { ikeys[i] = dis(re); }
        size_t pcount = 0;
        size_t icount = pre - 1;
        size_t dcount = 0;
        for (size_t i = pre; i < p_rep * 10 + pre; ++i)
        {
            if (pcount < pattern) { ikeys[++icount] = dis(re); }
            else
            {
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            otm::out() << otm::width(4) << i << otm::width(8) << alpha;
            table.print_init_data(otm::out());
            otm::out() << otm::width(9) << cap << otm::width(4) << pattern
                       << otm::width(9) << pre << otm::width(9) << n
                       << otm::width(8) << d_pre << otm::width(8) << d_mix
                       << otm::width(9) << errors
#ifdef MALLOC_COUNT
                       << otm::width(7)
                       << double(malloc_count_current() - (n + pre) * 8) /
                              (0.1 * double(pattern * 8 * 2 * n) +
                               double(8 * 2 * pre))
#endif
                       << std::endl;

            if (ferrors && (!errors))
                otm::out() << "Critical Error" << std::endl;
        }
...
int main(int argn, char** argc)
{
    ...
    if (c.bool_arg("-out") || c.bool_arg("-file"))
    {
        ...
        name             = c.str_arg("-file", name) + ".mixd";
        otm::out().set_file(name);
    }

    return Chooser::execute<Test, false>(c, it, n, n0, cap, pattern, steps,
                                         alpha);
}
