...
{
  public:
    history_raw(size_t s) { trend.reserve(s); }
...
struct Test
{
    ...
    int operator()(size_t it, size_t n, size_t steps, size_t seed)
    {
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            for (size_t i = 0; i < it; ++i)
            {
                // pop 1000 elements from the history
                auto start = history.trend.size() - 50;
                history.trend.resize(start);
                for (size_t j = start; j < n; ++j)
                {
                    // TODO this might fail with optimistic rwalk (pretty rare)
                    if (j < ii - 1) table.erase(keys[j]);
                    keys[j] = dis(re);
                }
                for (ii = start; i < n; ++ii)
                {
