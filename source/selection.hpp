...
struct Chooser
{
    ...
    {
        // return executeD<Functor, HistCount, DisRWalk>     ( c,
        // std::forward<Types>(param)...);
        ///*
        if (c.bool_arg("-bfs"))
            return executeD<Functor, HistCount,
                            dysect::cuckoo_displacement::bfs>(
                c, std::forward<Types>(param)...);
        else if (c.bool_arg("-rwalk"))
            return executeD<Functor, HistCount,
                            dysect::cuckoo_displacement::random_walk>(
                c, std::forward<Types>(param)...);

        std::cout << "ERROR: choose displacement Strategy (use triv)"
                  << std::endl;
        return executeD<Functor, HistCount,
                        dysect::cuckoo_displacement::trivial>(
            c, std::forward<Types>(param)...);
        //*/
    }
    ...
    executeD(utm::command_line_parser& c, Types&&... param)
    {
        ...
        {
        // case 64:
        //     return executeDT<Functor, HistCount, Displacer,   64> (c,
        //     std::forward<Types>(param)...);
        // case 128:
        //     return executeDT<Functor, HistCount, Displacer,  128> (c,
        //     std::forward<Types>(param)...);
        case 256:
            return executeDT<Functor, HistCount, Displacer, 256>(
                c, std::forward<Types>(param)...);
        // case 512:
        //     return executeDT<Functor, HistCount, Displacer,  512> (c,
        //     std::forward<Types>(param)...);
        // case 1024:
        //     return executeDT<Functor, HistCount, Displacer, 1024> (c,
        //     std::forward<Types>(param)...);
        // case 2048:
        //     return executeDT<Functor, HistCount, Displacer, 2048> (c,
        //     std::forward<Types>(param)...);
        // case 4096:
        //     return executeDT<Functor, HistCount, Displacer, 4096> (c,
        //     std::forward<Types>(param)...);
        default:
            constexpr auto ttl = dysect::cuckoo_config<>::tl;
            std::cout << "ERROR: unknown TL value (use " << ttl << ")"
                      << std::endl;
            return executeDT<Functor, HistCount, Displacer, ttl>(
                c, std::forward<Types>(param)...);
        }
    }
    ...
    executeDT(utm::command_line_parser& c, Types&&... param)
    {
        ...
        case 4:
            return executeDTB<Functor, HistCount, Displacer, TL, 4>(
                c, std::forward<Types>(param)...);
        // case 6:
        //     return executeDTB<Functor, HistCount, Displacer, TL,  6> (c,
        //     std::forward<Types>(param)...);
        case 8:
        ...
        default:
            ...
            std::cout << "ERROR: unknown BS value (use " << tbs << ")"
                      << std::endl;
            return executeDTB<Functor, HistCount, Displacer, TL, tbs>(
                c, std::forward<Types>(param)...);
        }
    }
    ...
    executeDTB(utm::command_line_parser& c, Types&&... param)
    {
        ...
        default:
            ...
            std::cout << "ERROR: unknown nh value (use " << tnh << ")"
                      << std::endl;
    ...
    {
        auto ns =
            c.int_arg("-ns", dysect::hopscotch_config<>::neighborhood_size);
        switch (ns)
        {
        case 64:
        ...
        default:
            ...
            std::cout << "ERROR: unknown ns value (use " << tns << ")"
                      << std::endl;
