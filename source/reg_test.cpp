...
template <class Config> struct Test
{
    ...
    {
        otm::out() << otm::width(4) << "# it" << otm::width(6) << "block"
                   << otm::width(8) << "alpha";
        table_type::print_init_header(out);
        otm::out() << otm::width(9) << "cap" << otm::width(9) << "n_full"
                   << otm::width(8) << "t_in" << otm::width(6) << "in_err"
#ifdef MALLOC_COUNT
                   << otm::width(7) << "memory"
#endif
                   << std::endl;
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            for (size_t j = 0; j < n / block_size; ++j)
            {
                ...
                for (size_t ti = j * block_size; ti < (j + 1) * block_size;
                     ++ti)
                {
                ...
                otm::out() << otm::width(4) << i << otm::width(6) << j
                           << otm::width(8) << alpha;
                table.print_init_data(out);
                otm::out() << otm::width(9) << cap << otm::width(9) << n
                           << otm::width(8) << d_step << otm::width(6)
                           << in_errors
#ifdef MALLOC_COUNT
                           << otm::width(7)
                           << double(malloc_count_current() - n * 8) /
