...
template <class Config> struct test_type
{
                   ...
                   double alpha)
    {
        ...
        otm::out() << otm::width(9) << "cap" << otm::width(9) << "n_win"
                   << otm::width(9) << "n_mix" << otm::width(8) << "t_win"
                   << otm::width(8) << "t_mix" << otm::width(8) << "t_eval"
                   << otm::width(9) << "op_err" << otm::width(9) << "ev_err"
#ifdef MALLOC_COUNT
                   << otm::width(7) << "mem"
#endif
                   << std::endl;
        ...
        for (size_t i = 0; i < n + win; ++i) { keys[i] = dis(re); }
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            otm::out() << otm::width(9) << cap << otm::width(9) << win
                       << otm::width(9) << n << otm::width(8) << d_win
                       << otm::width(8) << d_mix << otm::width(8) << d_eval
                       << otm::width(7) << del_errors + in_errors
                       << otm::width(7) << fin_errors
#ifdef MALLOC_COUNT
                       << otm::width(7)
                       << double(malloc_count_current() - 8 * (n + win)) /
                              double(8 * 2 * win)
#endif
                       << std::endl;
        }
...
int main(int argn, char** argc)
{
    ...
    {
        otm::out()
            << "This test measures access speed of deletions \n"
            << "\n"
            ...
            << "evaluate all previous moves" << std::endl;
        return 0;
    }
