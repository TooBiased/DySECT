...
struct test_type
{
    ...
    void
    prepare_find_keys(size_t* fikeys, size_t* inkeys, size_t fis, size_t ins)
    {
        std::uniform_int_distribution<uint64_t> pdis(1, ins);
        std::mt19937_64 re(ins * 2098740980990909098ull);
        for (size_t i = 0; i < fis; ++i) { fikeys[i] = inkeys[pdis(re)]; }
        ...
        size_t it, size_t n, size_t steps, double eps, double epstp, size_t win)
    {
        ...
        otm::out() << otm::width(9) << "n_step" << otm::width(5) << "win"
                   << otm::width(6) << "load" << otm::width(8) << "t_in"
                   << otm::width(8) << "t_fip" << otm::width(8) << "t_fin"
                   << otm::width(8) << "in_err" << otm::width(8) << "fi_err"
#ifdef MALLOC_COUNT
                   << otm::width(7) << "memory"
#endif
                   << std::endl;
        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            for (size_t i = 0; i < n; ++i) { inkeys[i] = dis(re); }
            ...
            while (nxt_blk + win < n)
            {
                ...
                otm::out() << otm::width(9) << nxt_blk << otm::width(5) << win
                           << otm::width(6) << tlf << otm::width(8) << d_in
                           << otm::width(8) << d_fip << otm::width(8) << d_fin
                           << otm::width(8) << in_errors << otm::width(8)
                           << fi_errors
#ifdef MALLOC_COUNT
                           << otm::width(7)
...
int main(int argn, char** argc)
{
    ...
    if (eps < 0.)
    {
        std::cout << "please input eps" << std::endl;
        return 8;
    }
