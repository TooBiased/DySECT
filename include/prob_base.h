...
template <class T> class prob_traits;
...
template <class SpProb> class prob_base
{
  private:
  ...
  public:
    ...
    {
        return static_cast<specialized_type*>(this)->cend();
    }
...
template <class SpProb>
inline int prob_base<SpProb>::displacement(const key_type& k) const
{
    auto ind = h(k);
...
template <class Specialized> class iterator_incr<prob_base<Specialized> >
{
  public:
  ...
  public:
    iterator_incr(const table_type& table_)
        : end_ptr(
              reinterpret_cast<ipointer>(&table_.table[table_.capacity - 1]))
    {
    }
