...
public:
    ProbBase(size_type cap, double alpha)
...
ProbBase<SpProb>::find(const key_type& k)
{
    ...
    {
        size_type ti = static_cast<Specialized_t*>(this)->mod(i);
        auto temp = table[ti];
...
ProbBase<SpProb>::find(const key_type& k) const
{
    ...
    for (size_type i = ind; ; ++i)
    {
        size_type ti = static_cast<const Specialized_t*>(this)->mod(i);
        auto temp = table[ti];
...
template<class SpProb>
inline typename ProbBase<SpProb>::size_type
ProbBase<SpProb>::erase(const key_type& k)
{
    auto ind = h(k);
...
template<class SpProb>
inline typename ProbBase<SpProb>::size_type
ProbBase<SpProb>::count(const key_type& k) const
{
...
template<class SpProb>
inline void ProbBase<SpProb>::propagate_remove(const size_type origin)
{
    size_type tempn = n;
    n = 0;
