...
template <class Parent> class dis_random_walk_cyclic
{
  private:
  ...
  public:
    dis_random_walk_cyclic(parent_type& parent, size_t steps = 256,
                           size_t seed = 30982391937209388ull)
    ...
    insert(value_intern t, hashed_type hash)
    {
        ...
        for (size_t i = 0; !tb->space() && i < steps; ++i)
        {
            ...
            if (tbd != tb)
                ...
                tb = tab.get_bucket(hash, nh - 1);

            queue.emplace_back(tp, tb);
        }

        if (tb->insert(tp)) { return std::make_pair(queue.size() - 1, pos); }
        ...
        for (size_t i = queue.size() - 2; i >= 1; --i)
        {
            ...
            if (!(tb->remove(tp.first))) { std::cout << "f1" << std::endl; }
