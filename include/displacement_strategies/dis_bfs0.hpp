...
template <class Parent> class dis_bfs0
{
  private:
  ...
  public:
    ...
    dis_bfs0(Parent& parent, dis_bfs0&& rhs) : tab(parent), steps(rhs.steps) {}

    inline std::pair<int, value_intern*>
    insert(std::pair<key_type, mapped_type> t, hashed_type hash)
    {
        bfs_queue bq;
        ...
        {
            bq.push_back(
                std::tuple<key_type, int, bucket_type*>(t.first, -1, b[i]));
        }
  ...
  private:
    ...
    inline value_intern*
    rollBackDisplacements(std::pair<key_type, mapped_type> t, bfs_queue& bq)
    {
        key_type     k1;
        int          prev1;
        bucket_type* b1;
