...
template <class Parent> class dis_random_walk_optimistic
{
  private:
  ...
  public:
    dis_random_walk_optimistic(parent_type& parent, size_t steps = 256,
                               size_t seed = 30982391937209388ull)
        : tab(parent), re(seed), steps(steps)
    {
    }

    dis_random_walk_optimistic(parent_type&                 parent,
                               dis_random_walk_optimistic&& rhs)
