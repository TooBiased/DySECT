...
template <size_t t_w, bool dpair> struct hash_value_splitter;
...
/* MAIN CLASS *****************************************************************/
template <class Key, class HFct, size_t TAB_WIDTH, size_t NH, bool DPAIR = true,
          bool LCOMB = true>
class hasher
{
  private:
    ...
    static constexpr size_t n_hfct =
        (DPAIR) ? (n_hpair >> 1) + (1 & n_hpair) : n_hpair;
  ...
  public:
    ...
    using extractor_type =
        hash_value_extractor<hashed_type, tab_w, DPAIR, LCOMB>;
  ...
  public:
    ...
    {
        hashed_type result;
        for (size_t i = 0; i < n_hfct; ++i) { result.hash[i] = fct[i](k); }
        return result;
    }
};
 ...
 * **********************************************/
template <size_t t_w> struct hash_value_splitter<t_w, true>
{
    uint64_t tab0 : t_w;
...
template <size_t t_w> struct hash_value_splitter<t_w, false>
{
    uint64_t tab : t_w;
    uint64_t loc : 64 - t_w;
};

template <> struct hash_value_splitter<0, true>
{
...
template <> struct hash_value_splitter<0, false>
{
...
class hash_value_extractor<Hashed, tab_width, true, false>
{
  public:
    inline static size_t tab(const Hashed& h, size_t i)
    {
        return (i & 1) ? h.split[i >> 1].tab1 : h.split[i >> 1].tab0;
    }
    inline static size_t loc(const Hashed& h, size_t i)
    {
        return (i & 1) ? h.split[i >> 1].loc1 : h.split[i >> 1].loc0;
    }
};
