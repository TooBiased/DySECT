...
class prob_quadratic
    : public prob_traits<prob_quadratic<K, D, HF, Conf> >::base_type
{
  private:
  ...
  public:
    ...
    prob_quadratic(size_type cap = 0, double size_constraint = 1.1,
                   size_type /*steps*/ = 0)
  ...
  private:
    ...
    inline void grow()
    {
        ...
        {
            auto temp = table[i];
            if (temp.first) { ntable.insert(temp); }
        }
...
prob_quadratic<K, D, HF, C>::find(const key_type& k)
{
    ...
    for (size_t i = 0;; ++i)
    {
        ...
        if (temp.first == 0) { return base_type::end(); }
        ind = next(ind, i);
    }
}
...
prob_quadratic<K, D, HF, C>::find(const key_type& k) const
{
    ...
    for (size_t i = 0;; ++i)
    {
        ...
        if (temp.first == 0) { return base_type::cend(); }
        ind = next(ind, i);
    }
}
...
template <class K, class D, class HF, class C>
inline int prob_quadratic<K, D, HF, C>::displacement(const key_type& k) const
{
    size_t ind = h(k);
...
template <class K, class D, class HF = utils_tm::hash_tm::default_hash,
          class Conf = triv_config>
class prob_quadratic_inplace
    : public prob_traits<prob_quadratic_inplace<K, D, HF, Conf> >::base_type
{
  private:
  ...
  public:
    prob_quadratic_inplace(size_type cap = 0, double size_constraint = 1.1,
                           size_type /*steps*/ = 0)
    ...
    inline void grow()
    {
        ...
        for (int i = ocap - 1; i >= 0; --i)
        {
            ...
            if (temp.first)
            {
                ...
                if (!intern_migration_insert(i, temp)) buffer.push_back(temp);
            }
        }
...
prob_quadratic_inplace<K, D, HF, C>::insert(const key_type&    k,
                                            const mapped_type& d)
{
    return insert(std::make_pair(k, d));
}
...
template <class K, class D, class HF, class C>
inline std::pair<typename prob_quadratic_inplace<K, D, HF, C>::iterator, bool>
prob_quadratic_inplace<K, D, HF, C>::insert(
    const std::pair<key_type, mapped_type>& t)
{
    size_t ind = h(t.first);
