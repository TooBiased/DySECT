    ...
    : public cuckoo_traits<cuckoo_independent_2lvl<K, D, HF, Conf> >::base_type
{
  ...
  private:
    ...
    inline void migrate(size_type tab, std::unique_ptr<bucket_type[]>& target,
                        size_type nsize, std::vector<value_intern>& grow_buffer)
    {
        ...
        for (size_type i = 0; i < csize; ++i)
        {
            ...
            for (size_type j = 0; j < bs; ++j)
            {
                ...
                for (size_type ti = 0; ti < nh; ++ti)
                {
                    ...
                    {
                        if (!target[fastrange(nsize, ext::loc(hash, ti))]
                                 .insert(e))
    ...
    {
        for (auto& e : grow_buffer) { base_type::insert(e); }
    }
};
...
class iterator_incr<cuckoo_independent_2lvl<K, D, HF, Conf> >
{
  ...
  private:
    ...
    ipointer overflow_tab()
    {
        ...
        end_tab     = reinterpret_cast<ipointer>(
            &table.ll_tab[tab][size].elements[bs - 1]);
    ...
    void initialize_tab(ipointer ptr)
    {
        ...
        {
            ipointer tab_b_ptr =
                reinterpret_cast<ipointer>(&table.ll_tab[i][0].elements[0]);
