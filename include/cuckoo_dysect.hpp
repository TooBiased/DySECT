...
class cuckoo_dysect
    : public cuckoo_traits<cuckoo_dysect<K, D, HF, Conf> >::base_type
{
  private:
  ...
  public:
        ...
        : base_type(size_constraint, dis_steps, seed)
    {
        ...
        while (avg_size_f > (size_small << 1)) size_small <<= 1;

        n_large =
            (size_small < avg_size_f)
    ...
    cuckoo_dysect(cuckoo_dysect&& rhs)
        : base_type(std::move(rhs)), n_large(rhs.n_large),
          bits_small(rhs.bits_small), bits_large(rhs.bits_large),
    ...
    cuckoo_dysect& operator=(cuckoo_dysect&& rhs)
    {
        ...
        for (size_type i = 0; i < tl; ++i) { std::swap(llt[i], rhs.llt[i]); }
        return *this;
    }
  ...
  private:
    ...
    inline void
    migrate_grw(size_type tab, std::unique_ptr<bucket_type[]>& target)
    {
        ...
        for (size_type i = 0; i < flag; ++i)
        {
            ...
            for (size_type j = 0; j < bs; ++j)
            {
                ...
                for (size_type ti = 0; ti < nh; ++ti)
                {
                    ...
                    if (ext::tab(hash, ti) == tab && (loc & bits_small) == i)
                    {
                        if (loc & flag)
                            tar1->elements[tj1++] = e;
                        else
    ...
    inline void
    migrate_shrnk(size_type tab, std::unique_ptr<bucket_type[]>& target,
                  std::vector<std::pair<key_type, mapped_type> >& buffer)
    {
        size_type flag = bits_small + 1;
    ...
    inline void
    finish_shrnk(std::vector<std::pair<key_type, mapped_type> >& buffer)
    {
        size_type err = 0;
        n -= buffer.size();
        for (auto& e : buffer) { err += (base_type::insert(e).second) ? 1 : 0; }
    }
};
...
class iterator_incr<cuckoo_dysect<K, D, HF, Conf> >
{
  ...
  private:
    ...
    ipointer overflow_tab()
    {
        ...
        size_type size =
            (tab < table.n_large) ? table.bits_large : table.bits_small;
        end_tab =
            reinterpret_cast<ipointer>(&table.llt[tab][size].elements[bs - 1]);
    ...
    void initialize_tab(ipointer ptr)
    {
        ...
        {
            ipointer tab_b_ptr =
                reinterpret_cast<ipointer>(&table.llt[i][0].elements[0]);
...
template <class K, class D, class HF = utils_tm::hash_tm::default_hash,
          class Conf = cuckoo_config<> >
class cuckoo_dysect_inplace
    : public cuckoo_traits<cuckoo_dysect_inplace<K, D, HF, Conf> >::base_type
{
  private:
  ...
  private:
    ...
    static constexpr size_type max_loc_size =
        max_size / tl / sizeof(bucket_type);
  ...
  private:
    ...
    // inline void migrate_shrnk(size_type tab, std::unique_ptr<bucket_type[]>&
    // target,
    ...
    // inline void finish_shrnk(std::vector<std::pair<key_type, mapped_type> >&
    // buffer)
    // {
    //     size_type bla = 0;
    //     n -= buffer.size();
  ...
  private:
    ...
    ipointer overflow_tab()
    {
        ...
        size_type size =
            (tab < table.n_large) ? table.bits_large : table.bits_small;
        end_tab =
            reinterpret_cast<ipointer>(&(tab_start + size)->elements[bs - 1]);
    ...
    void initialize_tab(ipointer ptr)
    {
        ...
        {
            auto tab_size =
                (i < table.n_large) ? table.bits_large : table.bits_small;
            auto bkt_last = tab_curr + tab_size;
            auto par_last =
                reinterpret_cast<ipointer>(&bkt_last->elements[bs - 1]);
