...
class cuckoo_base
{
  ...
  private:
    ...
    static constexpr bool      fix_errors =
        cuckoo_traits<specialized_type>::fix_errors;
  ...
  public:
    ...
    inline void clear()
    {
        auto temp = specialized_type(0, alpha);
        (*this)   = temp;
    }
  ...
  public:
    ...
    {
        out << otm::width(6) << "bsize" << otm::width(6) << "ntabl"
            << otm::width(6) << "nhash" << otm::width(9) << "f_cap"
            << std::flush;
    }

    void explicit_grow() { static_cast<specialized_type*>(this)->grow(); }
};
...
template <class SCuckoo>
inline int cuckoo_base<SCuckoo>::displacement(const key_type& k) const
{
    auto hash = hasher(k);
    int  disp = 0;
...
{
    return (static_cast<const specialized_type*>(this)->find(k) != cend()) ? 1
                                                                           : 0;
}
...
{
    out << otm::width(6) << bs << otm::width(6) << tl << otm::width(6) << nh
